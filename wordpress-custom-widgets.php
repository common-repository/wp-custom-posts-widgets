<?php
/*
Plugin Name: Wordpress Custom Posts  Widgets
Plugin URI: https://d3logics.com/contact/
Description: Wordpress Custom Widgets Plugin  for displaying posts, custom post types or sticky posts with custom  options.
Version: 1.0.1
Author: d3logics
 ...
 *
 * Wordpress Custom Posts  Widgets  is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation; either version 2 of the License, or
 * (at your option) any later version.
 *
 * Wordpress Custom Posts  Widgets  is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 */

if ( !class_exists( 'WP_Widget_DTLWP_Posts' ) ) {

  class WP_Widget_DTLWP_Posts extends WP_Widget {

    function __construct() {

      $widget_options = array(
        'classname' => 'widget_DTLWP_posts',
        'description' => __( 'Displays list of posts with an array of options', 'dtlwcpw' )
      );
      ...
      parent::__construct(
        'sticky-posts',
        __( 'WP Custom Posts  Widget', 'dtlwcpw' ),
        $widget_options,
        $control_options
      );

      $this->alt_option_name = 'widget_DTLWP_posts';
      ...
      if (apply_filters('dtlwcpw_enqueue_styles', true) && !is_admin()) {
        add_action('wp_enqueue_scripts', array(&$this, 'enqueue_theme_scripts'));
      }

      load_plugin_textdomain('dtlwcpw', false, basename( dirname( __FILE__ ) ) . '/languages' );

    }

    function enqueue_admin_scripts() {
      wp_register_style('dtlwcpw_admin_styles', plugins_url('css/dtlwcpw-admin.min.css', __FILE__));
      wp_enqueue_style('dtlwcpw_admin_styles');

      wp_register_script('dtlwcpw_admin_scripts', plugins_url('js/dtlwcpw-admin.min.js', __FILE__), array('jquery'), null, true);
      wp_enqueue_script('dtlwcpw_admin_scripts');
    }

    function enqueue_theme_scripts() {
      wp_register_style('dtlwcpw_theme_standard', plugins_url('css/dtlwcpw-theme-standard.min.css', __FILE__));
      wp_enqueue_style('dtlwcpw_theme_standard');
    }
    ...
    function widget( $args, $instance ) {
      ...
      $cache = wp_cache_get( 'widget_DTLWP_posts', 'widget' );
      ...
      $args = apply_filters('dtlwcpw_wp_query_args', $args, $instance, $this->id_base);

      $dtlwcpw_query = new WP_Query($args);

      if ($instance['template'] === 'custom') {
        $custom_template_path = apply_filters('dtlwcpw_custom_template_path',  '/dtlwcpw/' . $instance['template_custom'] . '.php', $instance, $this->id_base);
      ...
      wp_cache_set( 'widget_DTLWP_posts', $cache, 'widget' );
    }
    ...
    function update( $new_instance, $old_instance ) {
      ...
      if ( isset( $alloptions['widget_DTLWP_posts'] ) )
        delete_option( 'widget_DTLWP_posts' );
    ...
    function flush_widget_cache() {
      ...
      wp_cache_delete( 'widget_DTLWP_posts', 'widget' );
    ...
    function form( $instance ) {
      ...
      $instance = wp_parse_args( (array) $instance, array(
        'title' => __('Wp Custom  Posts Widget', 'dtlwcpw'),
        'class' => '',
        'title_link' => '' ,
        'number' => '5',
        'types' => 'post',
        ...
        'attag' => false,
        'excerpt_length' => 10,
        'excerpt_readmore' => __('Read more &rarr;', 'dtlwcpw'),
        'order' => 'DESC',
        'orderby' => 'date',
        'meta_key' => '',
        'sticky' => 'show',
        ...
        'show_thumbnail' => true,
        'custom_fields' => '',
        // Set template to 'legacy' if field from dtlwcpw < 2.0 is set.
      ...
      <div class="dtlwcpw-tabs">
        <a class="dtlwcpw-tab-item active" data-toggle="dtlwcpw-tab-general"><?php _e('General', 'dtlwcpw'); ?></a>
        <a class="dtlwcpw-tab-item" data-toggle="dtlwcpw-tab-display"><?php _e('Display', 'dtlwcpw'); ?></a>
        <a class="dtlwcpw-tab-item" data-toggle="dtlwcpw-tab-filter"><?php _e('Filter', 'dtlwcpw'); ?></a>
        <a class="dtlwcpw-tab-item" data-toggle="dtlwcpw-tab-order"><?php _e('Order', 'dtlwcpw'); ?></a>
      </div>

      <div class="dtlwcpw-tab dtlwcpw-tab-general">

        <p>
          <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'dtlwcpw' ); ?>:</label>
          <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" />
        </p>

        <p>
          <label for="<?php echo $this->get_field_id( 'title_link' ); ?>"><?php _e( 'Title URL', 'dtlwcpw' ); ?>:</label>
        ...
        <p>
          <label for="<?php echo $this->get_field_id( 'class' ); ?>"><?php _e( 'CSS class', 'dtlwcpw' ); ?>:</label>
          <input class="widefat" id="<?php echo $this->get_field_id( 'class' ); ?>" name="<?php echo $this->get_field_name( 'class' ); ?>" type="text" value="<?php echo $class; ?>" />
        </p>

        <p>
          <label for="<?php echo $this->get_field_id('before_posts'); ?>"><?php _e('Before posts', 'dtlwcpw'); ?>:</label>
        ...
        <p>
          <label for="<?php echo $this->get_field_id('after_posts'); ?>"><?php _e('After posts', 'dtlwcpw'); ?>:</label>
      ...
      <div class="dtlwcpw-tab dtlwcpw-hide dtlwcpw-tab-display">

        <p>
          <label for="<?php echo $this->get_field_id('template'); ?>"><?php _e('Template', 'dtlwcpw'); ?>:</label>
          <select name="<?php echo $this->get_field_name('template'); ?>" id="<?php echo $this->get_field_id('template'); ?>" class="widefat">
            <option value="legacy"<?php if( $template == 'legacy') echo ' selected'; ?>><?php _e('Legacy', 'dtlwcpw'); ?></option>
            <option value="standard"<?php if( $template == 'standard') echo ' selected'; ?>><?php _e('Standard', 'dtlwcpw'); ?></option>
            <option value="custom"<?php if( $template == 'custom') echo ' selected'; ?>><?php _e('Custom', 'dtlwcpw'); ?></option>
          </select>
        </p>

        <p<?php if ($template !== 'custom') echo ' style="display:none;"'; ?>>
          <label for="<?php echo $this->get_field_id('template_custom'); ?>"><?php _e('Custom Template Name', 'dtlwcpw'); ?>:</label>
        ...
        <p>
          <label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of posts', 'dtlwcpw' ); ?>:</label>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id( 'show_title' ); ?>"><?php _e( 'Show title', 'dtlwcpw' ); ?></label>
        </p>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id( 'show_date' ); ?>"><?php _e( 'Show published date', 'dtlwcpw' ); ?></label>
        </p>

        <p<?php if (!$show_date) echo ' style="display:none;"'; ?>>
          <label for="<?php echo $this->get_field_id('date_format'); ?>"><?php _e( 'Date format', 'dtlwcpw' ); ?>:</label>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id( 'show_author' ); ?>"><?php _e( 'Show post author', 'dtlwcpw' ); ?></label>
        </p>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id( 'show_comments' ); ?>"><?php _e( 'Show comments count', 'dtlwcpw' ); ?></label>
        </p>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id( 'show_excerpt' ); ?>"><?php _e( 'Show excerpt', 'dtlwcpw' ); ?></label>
        </p>

        <p<?php if (!$show_excerpt) echo ' style="display:none;"'; ?>>
          <label for="<?php echo $this->get_field_id('excerpt_length'); ?>"><?php _e( 'Excerpt length (in words)', 'dtlwcpw' ); ?>:</label>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id( 'show_content' ); ?>"><?php _e( 'Show content', 'dtlwcpw' ); ?></label>
        </p>
        ...
        <p<?php if (!$show_excerpt && !$show_content) echo ' style="display:none;"'; ?>>
          ...
          <?php _e( 'Show read more link', 'dtlwcpw' ); ?>
          </label>
        </p>
        ...
        <?php if ( function_exists('the_post_thumbnail') && current_theme_supports( 'post-thumbnails' ) ) : ?>
          ...
          <p>
            ...
            <label for="<?php echo $this->get_field_id( 'show_thumbnail' ); ?>"><?php _e( 'Show thumbnail', 'dtlwcpw' ); ?></label>
          </p>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('show_cats'); ?>"> <?php _e('Show post categories', 'dtlwcpw'); ?></label>
        </p>
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('show_tags'); ?>"> <?php _e('Show post tags', 'dtlwcpw'); ?></label>
        </p>

        <p>
          <label for="<?php echo $this->get_field_id( 'custom_fields' ); ?>"><?php _e( 'Show custom fields (comma separated)', 'dtlwcpw' ); ?>:</label>
      ...
      <div class="dtlwcpw-tab dtlwcpw-hide dtlwcpw-tab-filter">
        ...
        <p>
          ...
          <label for="<?php echo $this->get_field_id('atcat'); ?>"> <?php _e('Show posts only from current category', 'dtlwcpw');?></label>
        </p>

        <p>
          <label for="<?php echo $this->get_field_id('cats'); ?>"><?php _e( 'Categories', 'dtlwcpw' ); ?>:</label>
        ...
        <?php if ($tag_list) : ?>
          <p>
            ...
            <label for="<?php echo $this->get_field_id('attag'); ?>"> <?php _e('Show posts only from current tag', 'dtlwcpw');?></label>
          </p>

          <p>
            <label for="<?php echo $this->get_field_id('tags'); ?>"><?php _e( 'Tags', 'dtlwcpw' ); ?>:</label>
        ...
        <p>
          <label for="<?php echo $this->get_field_id('types'); ?>"><?php _e( 'Post types', 'dtlwcpw' ); ?>:</label>
        ...
        <p>
          <label for="<?php echo $this->get_field_id('sticky'); ?>"><?php _e( 'Sticky posts', 'dtlwcpw' ); ?>:</label>
          <select name="<?php echo $this->get_field_name('sticky'); ?>" id="<?php echo $this->get_field_id('sticky'); ?>" class="widefat">
            <option value="show"<?php if( $sticky === 'show') echo ' selected'; ?>><?php _e('Show All Posts', 'dtlwcpw'); ?></option>
            <option value="hide"<?php if( $sticky == 'hide') echo ' selected'; ?>><?php _e('Hide Sticky Posts', 'dtlwcpw'); ?></option>
            <option value="only"<?php if( $sticky == 'only') echo ' selected'; ?>><?php _e('Show Only Sticky Posts', 'dtlwcpw'); ?></option>
          </select>
        </p>

      </div>

      <div class="dtlwcpw-tab dtlwcpw-hide dtlwcpw-tab-order">

        <p>
          <label for="<?php echo $this->get_field_id('orderby'); ?>"><?php _e('Order by', 'dtlwcpw'); ?>:</label>
          <select name="<?php echo $this->get_field_name('orderby'); ?>" id="<?php echo $this->get_field_id('orderby'); ?>" class="widefat">
            <option value="date"<?php if( $orderby == 'date') echo ' selected'; ?>><?php _e('Published Date', 'dtlwcpw'); ?></option>
            <option value="title"<?php if( $orderby == 'title') echo ' selected'; ?>><?php _e('Title', 'dtlwcpw'); ?></option>
            <option value="comment_count"<?php if( $orderby == 'comment_count') echo ' selected'; ?>><?php _e('Comment Count', 'dtlwcpw'); ?></option>
            <option value="rand"<?php if( $orderby == 'rand') echo ' selected'; ?>><?php _e('Random'); ?></option>
            <option value="meta_value"<?php if( $orderby == 'meta_value') echo ' selected'; ?>><?php _e('Custom Field', 'dtlwcpw'); ?></option>
            <option value="menu_order"<?php if( $orderby == 'menu_order') echo ' selected'; ?>><?php _e('Menu Order', 'dtlwcpw'); ?></option>
          </select>
        </p>

        <p<?php if ($orderby !== 'meta_value') echo ' style="display:none;"'; ?>>
          <label for="<?php echo $this->get_field_id( 'meta_key' ); ?>"><?php _e('Custom field', 'dtlwcpw'); ?>:</label>
        ...
        <p>
          <label for="<?php echo $this->get_field_id('order'); ?>"><?php _e('Order', 'dtlwcpw'); ?>:</label>
          <select name="<?php echo $this->get_field_name('order'); ?>" id="<?php echo $this->get_field_id('order'); ?>" class="widefat">
            <option value="DESC"<?php if( $order == 'DESC') echo ' selected'; ?>><?php _e('Descending', 'dtlwcpw'); ?></option>
            <option value="ASC"<?php if( $order == 'ASC') echo ' selected'; ?>><?php _e('Ascending', 'dtlwcpw'); ?></option>
          </select>
        </p>

      </div>

      <p class="dtlwcpw-credits">
        <?php _e('Enjoy this plugin? <a href="https://d3logics.com/contact/" target="_blank">Send a tip to support development</a>.', 'dtlwcpw'); ?>
      </p>
  ...
  function init_wp_widget_DTLWP_posts() {
    register_widget( 'WP_Widget_DTLWP_Posts' );
  }

  add_action( 'widgets_init', 'init_wp_widget_DTLWP_posts' );
}
